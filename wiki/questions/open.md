...
`npm run sim` of dumb-vs-dumb produces 100% draws at the tick limit. Still 100% on the new ~1000-cell hex board, suggesting the heuristic's failure mode is structural (front-line oscillation), not a function of graph size. Trace on the bootstrap graph shows the cause; both sides capture their adjacent neutrals quickly, then the front line oscillates because:
