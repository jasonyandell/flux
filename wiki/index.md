...
- [[topics/v2-edge-voting-policy|v2-edge-voting-policy]] — spec for an edge-centric local-flow representation: derived edge types, distributed edge votes, multi-signal flow channels, and pulse-preserving hold/release learning. Spatial layer; pair with [[topics/v2-temporal-strategy]] for the temporal layer.
- [[topics/v2-temporal-strategy|v2-temporal-strategy]] — ML-scientist synthesis (2026-05-15). Throttle (pressure/waste cliff), target dithering (multi-enemy cycling), and the strategic vocabulary (migrate/spread/flank/chase/corner) are one options-framework problem. Strategies map to measurable graph quantities; architecture is a slow KL-hysteresis manager over `(target, throttle, intent_vector)` with a fast goal-conditioned worker. Bandit-with-switching-costs diagnosis for the dithering pathology. AlphaStar / FeUdal / HER lineage. **Validation (same day):** `lightning_sum_throttled` dominates both bfs (7/7 coherent, p≈0.016) and vanilla `lightning_sum` (9/9, p≈0.004) at R=25 P=12 40%-dead.
