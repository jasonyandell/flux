...
Optional `throttle: int` kwarg caps each cell's desired outflows to the
top-`throttle` slots (attack-tier first, then by pot). See
`wiki/topics/v2-temporal-strategy.md` for the validation that
`sum + throttle=1` beats both bfs and vanilla sum at R=25 P=12 40%-dead.
...
def lightning_solver_actions(
    ...
    **mode_kwargs,
    ...
    return lightning_actions(
        ...
        mode=mode, curl_dir=curl_dir, throttle=throttle, **mode_kwargs,
    )
