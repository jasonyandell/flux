...
def apply_actions(state: State, actions: np.ndarray) -> State:
    ...
    for (c, k) in pending_on:
        ...
        opp = int(back[c, k])
        if opp < 0 or not new_outflow[d, opp]:
...
def tick(state: State) -> State:
    ...
    # where d = neighbors[c, k].  d → c slot is back[c, k] (per-cell on
    # non-hex topologies; OPPOSITE_SLOT[k] on hex grids).
    for k in range(K):
        d_ids = nb[:, k]                                   # (N,)
        valid = d_ids >= 0
        back_k = back[:, k]                                # (N,) per-cell back slot
        # Pressure flowing from d into c along its back-edge slot.
        # Only counts if d had outflow[d, back_k] = True (otherwise it's 0).
        pressure = np.zeros(N, dtype=np.float32)
        valid_d = d_ids[valid]
        valid_back = back_k[valid]
        pressure[valid] = edge_pressure_prev[valid_d, valid_back]
...
def waste_per_cell_for_tick(state: State) -> np.ndarray:
    ...
    back = back_slot_or_default(state)
    is_alive = owner >= 0
    ...
    for k in range(K):
        ...
        pressure = np.zeros(N, dtype=np.float32)
        valid_d = d_ids[valid]
        pressure[valid] = edge_pressure_prev[valid_d, back_k[valid]]
        d_owner = np.full(N, NEUTRAL, dtype=np.int32)
        d_owner[valid] = owner[valid_d]
        is_friendly_d = valid & (d_owner == owner) & is_alive
        pressure_in_friendly += pressure * is_friendly_d.astype(np.float32)
