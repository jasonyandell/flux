...
from .state import back_slot_or_default
...
def _inbound_enemy_pressure(state: State, seat: int) -> np.ndarray:
    ...
    for k in range(K):
        ...
        press[valid] = ep[valid_d, back_k[valid]]
...
def compute_potential(
    ...
    if mode == "sum":
        ...
        # slot back[c, k] on d, so pressure d→c = ep[d, back[c, k]].
        back = back_slot_or_default(state)
        # back may be -1 for empty slots; mask those out below via nbr_ok_f.
        back_safe = np.maximum(back, 0)
        ep_dc = ep[nb_safe, back_safe]                           # (N, K)
...
def _chase_actions(
    ...
    for k in range(K):
        ...
        valid_d = d_ids[valid]
        press = np.zeros(N, dtype=np.float32)
        press[valid] = edge_pressure[valid_d, back_k[valid]]
        d_owner = np.full(N, NEUTRAL, dtype=np.int32)
        d_owner[valid] = owner[valid_d]
